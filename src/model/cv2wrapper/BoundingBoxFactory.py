...
def get_bounding_box_list(detection_results, detection_threshold=0.5) -> list:
    bounding_boxes = list()

    for i in range(len(detection_results["detection_scores"])):
        confidence = detection_results["detection_scores"][i]
        debug_print("i: ", i)
        debug_print("detection_results[\"detection_scores\"]: ", detection_results["detection_scores"][i])
        if confidence > detection_threshold:
            ...
            debug_print(new_box, ", Confidence: ", confidence)
